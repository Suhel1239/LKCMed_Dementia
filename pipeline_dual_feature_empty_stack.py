"""
=============================================================================
Tri-Modal Fusion Pipeline  --  Dual Feature Score CSV  (Empty-Stack Support)
=============================================================================
  ...
  Fusion: concat active branches -> LayerNorm -> MLP -> 2 classes

  EMPTY STACK SUPPORT:
    Set SLICES_ENFACE=[] to disable the En face branch entirely.
    Set SLICES_OCTA=[]   to disable the OCTA branch entirely.
    The model detects empty lists at build time, skips the ViT encoder for
    that branch, and shrinks fused_dim accordingly — no crash, no zero padding.
...
Run (fresh)  : python pipeline_dual_feature_empty_stack.py
Run (resume) : python pipeline_dual_feature_empty_stack.py --resume
=============================================================================
"""
...
def load_slices(subject_id, folder_map, slice_names, transform):
    """Returns (N, 3, 224, 224). Returns empty tensor (0, 3, 224, 224) when slice_names=[]."""
    if not slice_names:
...
class TriModalFusionModel(nn.Module):
    def __init__(self, bio_dim: int, bio_embed: int = 256,
        ...
        active = ([f"EnFace({VIT_DIM})"] if use_enface else []) + \
                 ([f"OCTA({VIT_DIM})"]   if use_octa   else []) + \
                 [f"Bio({bio_embed})"]
        n = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print(f"[TriModalFusion] {' + '.join(active)} -> fused={fused_dim}  trainable={n:,}")
    ...
    def forward(self, enface, octa, bio):
        ...
        if self.use_enface:
            ...
            parts.append(self.enface_vit(enface.view(B*Ne, C, H, W))
                             .view(B, Ne, -1).mean(dim=1))
        if self.use_octa:
            No = octa.shape[1]
            C, H, W = octa.shape[2], octa.shape[3], octa.shape[4]
            parts.append(self.octa_vit(octa.view(B*No, C, H, W))
...
def run_kfold(subject_ids, df_bio, y, label_names, selected,
    ...
    enface_str = str(SLICES_ENFACE) if use_enface else "DISABLED (empty list)"
    octa_str   = str(SLICES_OCTA)   if use_octa   else "DISABLED (empty list)"
    print("\n" + "=" * 70)
