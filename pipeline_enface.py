"""
Pipeline: En face OCT images + OCT biomarkers -> ViT-B/16 Fusion -> 5-Fold CV

Run:
    python pipeline_enface.py m    # mean pooling across 9 slices (default)
    python pipeline_enface.py t    # temporal transformer across 9 slices
"""
...
def parse_mode():
    ...
    parser.add_argument("mode", nargs="?", default="m", choices=["m", "t"])
    args, _ = parser.parse_known_args()
    return args.mode.lower()


def run_kfold(subject_ids, df_bio, y, label_names,
              selected_features, folder_map, use_temporal, weights_dir):
    ...
    print("\n" + "=" * 70)
    print(f"  EN FACE OCT  |  {CFG['n_splits']}-Fold CV  |  "
          f"{'Transformer' if use_temporal else 'Mean Pool'}  |  "
    ...
    for fold, (tr_idx, ts_idx) in enumerate(
        ...
        best_path = os.path.join(weights_dir, f"fold{fold}_best.pth")
        last_path = os.path.join(weights_dir, f"fold{fold}_last.pth")
...
def main():
    mode         = parse_mode()
    use_temporal = (mode == "t")
    mode_label   = "temporal_transformer" if use_temporal else "mean_pooling"
    weights_dir  = os.path.join(WEIGHT_BASE, f"enface_{mode_label}")
    os.makedirs(weights_dir, exist_ok=True)

    print(f"[pipeline_enface]  mode={mode}  device={device}")
    ...
    run_kfold(subject_ids, df_bio, y, label_names,
              selected, folder_map, use_temporal, weights_dir)


if __name__ == "__main__":
    mode      = parse_mode()
    log_label = "temporal_transformer" if mode == "t" else "mean_pooling"
    sys.stdout = Tee(os.path.join(LOG_DIR,
                                  f"results_enface_{log_label}.txt"))
    try:
        main()
    finally:
        sys.stdout.close()
